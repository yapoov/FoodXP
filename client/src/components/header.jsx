...
function Header({ title }) {
  ...
  useEffect(() => {
    ...
    const getNotifications = async () => {
      try {
        ...
        setNotifs(res.data.data.expired);
        console.log(notifs);
      } catch (e) {
        console.log(e);
      }
  ...
  return (
    <div className="w-full relative bg-teal-300  h-48 flex flex-col justify-end">
      <BackButton />
      {user && (
        <span className=" text-white pl-4  text-2xl">
          {" "}
      ...
      <Link
        to={"/notifications"}
        className=" absolute top-4 right-4 rounded-full bg-teal-200 p-2"
      >
        {notifs.length !== 0 && (
          <div className=" animate-bounce rounded-full text-balance text-center bg-red-200 text-gray-600 font-bold w-6 h-6  absolute -top-1 -left-1">
            {notifs.length}
          </div>
        )}
        <svg
